...
import { X, Sparkles, AlertTriangle, ExternalLink, UserCheck } from "lucide-react"
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
      <div
        ...
        onClick={onClose}
      />
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-xl flex-col overflow-hidden bg-card shadow-xl">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                可信度 <span className="font-medium text-foreground">{skill.confidence_score}</span>
              </span>
            </div>
          </div>
          <button
            ...
            <section className="border-b border-border px-6 py-4">
              <div className="flex items-start gap-3 rounded-lg border border-teal-200 bg-teal-50/60 p-3">
                ...
                <div className="flex flex-col gap-1">
                  <div className="text-xs font-medium text-teal-900">AI 解释摘要</div>
                  <p className="text-sm leading-relaxed text-slate-700">{skill.ai_summary}</p>
                </div>
              </div>
            </section>
          ) : null}
          ...
          {skill.manager_comment ? (
            <section className="border-b border-border px-6 py-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex flex-col gap-1">
                  <div className="text-xs font-medium text-foreground">经理意见</div>
                  <p className="text-sm leading-relaxed text-slate-700">{skill.manager_comment}</p>
                </div>
              </div>
            </section>
          ) : null}
          ...
          <section className="px-6 py-4">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-foreground">
                证据明细 <span className="text-muted-foreground font-normal">({skill.evidence.length})</span>
              </h3>
            </div>
                ...
                {skill.evidence.map((ev) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <span>
                        置信度 <span className="text-foreground">{Math.round(ev.confidence_score * 100)}</span>
                      </span>
        ...
        <footer className="flex items-center justify-between border-t border-border bg-slate-50/60 px-6 py-3">
          <span className="text-xs text-muted-foreground">最近更新 {skill.last_updated_at}</span>
          <div className="flex items-center gap-2">
            <button className="rounded-md border border-border bg-card px-3 py-1.5 text-sm font-medium hover:bg-muted">
              补充证据
            </button>
