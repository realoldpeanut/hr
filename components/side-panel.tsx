    ...
    <aside className="flex flex-col gap-4">
        ...
        <ul className="flex flex-col divide-y divide-border/70">
          {todos.map((t) => {
            ...
            return (
              ...
              >
                <span className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full ${toneMap[t.tone]}`} />
      ...
      <section className="rounded-xl border border-border bg-card">
        <header className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <span className="text-[10px] text-muted-foreground">仅用于总结与提示</span>
        </header>
        <div className="flex flex-col gap-2 p-3">
          ...
          <button className="inline-flex items-center justify-center gap-1 rounded-md border border-border bg-card px-3 py-2 text-xs font-medium text-foreground transition hover:bg-muted">
            让 AI 总结我的技能档案
        ...
        <dl className="flex flex-col divide-y divide-border/70">
          {rules.map((r) => (
            <div key={r.title} className="px-4 py-3">
              <dt className="text-xs font-semibold text-foreground">{r.title}</dt>
              <dd className="mt-1 text-xs leading-relaxed text-muted-foreground">{r.text}</dd>
            </div>
          ))}
        </dl>
      </section>
